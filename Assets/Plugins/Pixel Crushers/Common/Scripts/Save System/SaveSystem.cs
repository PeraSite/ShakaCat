...
namespace PixelCrushers
{
    ...
    public class SaveSystem : MonoBehaviour
    {
        ...
        {
            get { return !m_instance.SafeIsUnityNull(); }
        }
        ...
        /// <summary>
        /// Invoked after ResetGameState() has been called.
        /// </summary>
        public static event System.Action restartGameEnded = delegate { };
        ...
        public static async UniTask LoadSceneInternalTransitionAsync(string sceneName) {
            await sceneTransitionManager.LeaveScene();
            if (sceneName.StartsWith("index:"))
            {
                var index = SafeConvert.ToInt(sceneName.Substring("index:".Length));
            ...
            await sceneTransitionManager.EnterScene();
        }

        public static IEnumerator LoadAdditiveSceneInternal(string sceneName)
        {
            yield return UnityEngine.SceneManagement.SceneManager.LoadSceneAsync(sceneName, UnityEngine.SceneManagement.LoadSceneMode.Additive).ToUniTask();
        ...
        public static async UniTask LoadAdditiveSceneInternalAsync(string sceneName)
        {
            await UnityEngine.SceneManagement.SceneManager.LoadSceneAsync(sceneName,
            ...
            foreach (var go in rootGOs) {
                RecursivelyApplySavers(go.transform);
            }
        }
        ...
        public static async UniTask SaveToSlotAsync(int slotNumber)
        {
            saveStarted();
            await UniTask.NextFrame();
            PlayerPrefs.SetInt(LastSavedGameSlotPlayerPrefsKey, slotNumber);
            await storer.StoreSavedGameDataAsync(slotNumber, RecordSavedGameData()).ToUniTask(instance);
            saveEnded();
        }
        ...
        public static IEnumerator LoadFromSlotCoroutine(int slotNumber)
        {
            loadStarted();
            yield return null;
            LoadFromSlotNow(slotNumber);
        ...
        /// </summary>
        /// <param name="sceneName">Scene to additively load.</param>
        public static async UniTask LoadAdditiveSceneAsync(string sceneName)
        {
            if (string.IsNullOrEmpty(sceneName) || m_addedScenes.Contains(sceneName)) return;
            m_addedScenes.Add(sceneName);
            instance.m_isLoadingAdditiveScene = true;
            await LoadAdditiveSceneInternalAsync(sceneName);
        }
